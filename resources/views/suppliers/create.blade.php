        ...
        <x-common.component-card title="Datos del Proveedor">
            <form method="POST" action="{{ route('suppliers.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Nombre del proveedor"
                ...
                <div>
                    ...
                    <input type="text" name="nit" value="{{ old('nit') }}" placeholder="Ej: 123456789"
                ...
                <div>
                    ...
                    <textarea name="contact_info" rows="3" placeholder="Teléfono, email, dirección..."
                        class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('contact_info') }}</textarea>
                    @error('contact_info')<p class="mt-1 text-xs text-error-500">{{ $message }}</p>@enderror
                </div>

                {{-- Active toggle --}}
                <div x-data="{ active: {{ old('active', true) ? 'true' : 'false' }} }">
